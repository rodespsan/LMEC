 ...
 */
class CExistInArrayValidator extends CValidator {
    ...
    protected function validateAttribute($object, $attribute) {
        $arrayValues = $object->$attribute;
        if ($this->allowEmpty && $this->isEmpty($arrayValues))
            return;
        ...
        foreach ($arrayValues as $value) {
            ...
            if (!$finder->exists($criteria)) {
                $hasError = true;
                $message .= ($message === null) ? Yii::t('yii', '{attribute} "' . CHtml::encode($value) . '"') : Yii::t('yii', 'y "' . CHtml::encode($value) . '"');
//                $this->addError($object, $attribute, $message, array('{value}' => CHtml::encode($value)));
        ...
        if ($hasError) {
            $message .= Yii::t('yii', ' es (son) inválido(s)');
